...
import { useCompleteCourseMutation, useGetCourseProgressQuery, useIncompleteCourseMutation, useUpdateLectureProgressMutation } from "@/features/api/courseProgressApi.js";
...
const CourseProgress = () => {
  ...
  }, [completedSuccess, incompleteCourse]);
  ...
  const handleCompleteCourse = async (req,res) => {
      await completeCourse(courseId)
      refetch()
}
const handleIncompleteCourse = async (req,res) => {
    ...
    <div className="max-w-7xl mx-auto p-4 mt-20">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <div className="flex-1 md:w-3/5 h-fit rounded-lg shadow-lg p-4">
          <div>{/* <video /> */}</div>
          {/* Display current wathcing lecture title */}
            <video
            controls
            className="w-full h-auto md:rounded-lg"
            src={getSafeUrl(currentLecture?.videoUrl || initialLecture.videoUrl)}
